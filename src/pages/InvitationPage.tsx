  ...
  return (
    <Card>
      <div className="bg-cover bg-center h-full flex flex-col items-center justify-center text-center">
        ...
        <p className='flex-1 mt-2 break-keep'>
          <span className={' duration-1000 transition-all ' + effectToggle(show)}>싱그러운 초록으로 짙어가는 이 여름 저희 결혼합니다. <br className='hidden md:inline' /></span>
          <span className={' duration-1000 transition-all ' + effectToggle(show)}>변하지 않는 사랑으로 부부의 깊은 연을 맺고 한길을 걷고자 합니다. <br className='hidden md:inline' /></span>
          <span className={' duration-1000 transition-all ' + effectToggle(show)}>그 시작의 자리에 함께해 주시길 바랍니다.</span>
        </p>
